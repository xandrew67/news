...
Coordinates the full pipeline:
  FeedAgent → SummaryAgent → AnalysisAgent → NewsJudge → PostAgent → PrePostReviewEditPublish

Generated posts are queued for human review via the web UI before publishing.
Run `python main.py --serve` (or `python web_app.py`) to open the review interface.
"""
...
class Orchestrator:
    ...
    def __init__(self, settings_path: str = "config/settings.yaml",
        ...
        self.analysis_agent = AnalysisAgent(self.client, model, max_tokens=4096)
        ...
        self.review_queue = PrePostReviewEditPublish()
    ...
    def run_once(self) -> list[XPost]:
        ...
        # Step 6: Queue for human review (replaces direct X posting)
        self.review_queue.enqueue(posts, scored)
